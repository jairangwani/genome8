...
if (command === 'stop') {
    ...
    for (const child of pids.children || []) {
        try {
            ...
            console.log(`  ${child.name} (pid: ${child.pid}) already dead`);
        }
    }
    try {
        if (process.platform === 'win32') {
        ...
        console.log(`  Main process already dead`);
    }
...
if (command === 'ps') {
    ...
    if (fs.existsSync(stateFile)) {
        ...
        if (state.stats)
            console.log(`  Graph: ${state.stats.total_nodes}n, ${state.stats.total_journeys}j, ${state.stats.total_connections}c`);
    }
    process.exit(0);
}
...
if (command === 'compile') {
    ...
    fs.writeFileSync(path.join(compiledDir, 'index.yaml'), yaml.dump(result.index, { lineWidth: 120, noRefs: true }));
    fs.writeFileSync(path.join(compiledDir, 'coverage.yaml'), yaml.dump(result.coverage, { lineWidth: 120, noRefs: true }));
...
if (command === 'excerpt') {
    ...
    if (!moduleName) {
        console.error('Usage: genome excerpt <dir> <module> [round]');
        process.exit(1);
    }
    const filePath = path.join(modulesDir, `${moduleName}.yaml`);
    if (!fs.existsSync(filePath)) {
        console.error(`Module file not found: ${filePath}`);
        process.exit(1);
    }
...
if (command === 'status') {
    ...
    if (!converged) {
        ...
        if (s.orphans > 0)
            console.log(`  → Connect ${s.orphans} orphan nodes to journeys`);
        if (s.isolated_modules > 0)
            console.log(`  → Add cross-module journey steps for ${s.isolated_modules} isolated modules`);
        if (s.duplicate_names > 0)
            console.log(`  → Rename ${s.duplicate_names} duplicate nodes`);
    }
}
if (command === 'publish') {
...
if (command === 'test-gen') {
    ...
    if (!testDir) {
        console.error('Usage: genome test-gen <modules-dir> <test-dir>');
        process.exit(1);
    }

...
if (process.env.GENOME_MAX_CONCURRENT_CHILDREN)
    config.maxConcurrentChildren = parseInt(process.env.GENOME_MAX_CONCURRENT_CHILDREN);
if (process.env.GENOME_MAX_HIERARCHY_DEPTH)
    config.maxHierarchyDepth = parseInt(process.env.GENOME_MAX_HIERARCHY_DEPTH);
if (process.env.GENOME_MESSAGE_TIMEOUT_MS)
    config.messageTimeoutMs = parseInt(process.env.GENOME_MESSAGE_TIMEOUT_MS);
...
async function retryOnLLMTimeout(prompt, maxRetries = 3) {
            ...
            if (state.status === 'sleeping' || state.status === 'unstable') {
                ...
                if (currentHash === state.interface_hash && specUnchanged) {
                    ...
                    if (skipPipeline) {
                        ...
                        const interface_ = { version_hash: currentHash, provides: {} };
                        ...
                        if (fs.existsSync(depsFilePath)) {
                            ...
                            const deps = yaml.load(fs.readFileSync(depsFilePath, 'utf-8'));
                        ...
                        const specWatcherResumed = fs.watch(path.dirname(specPath), (eventType, filename) => {
                            if (filename === 'spec.md') {
                                ...
                                const savedSpecHash = (() => { try {
                                    return JSON.parse(fs.readFileSync(path.join(internalDir, 'convergence-state.json'), 'utf-8')).spec_hash;
                                }
                                catch {
                                    return null;
                                } })();
...
Use the Write tool to write the file NOW.`);
        ...
        trackLLMCall();
        trackLLMCall();
        trackLLMCall(); // 4 calls: 3 angles + merge
    ...
    if (fs.existsSync(enginesDir)) {
        const existingEngines = fs.readdirSync(enginesDir).filter(d => fs.statSync(path.join(enginesDir, d)).isDirectory());
...
Write the file NOW using the Write tool.`);
    ...
    let affectedModules = null; // null = all modules, string[] = only these
...
7. If nothing is genuinely missing from this perspective, respond with "No changes needed" and do NOT edit the file`);
    ...
    while (true) {
        ...
        const orphans = Object.entries(result.index.nodes).filter(([, n]) => n.in_journeys.length === 0 && n.preceded_by.length === 0 && n.followed_by.length === 0);
        ...
        if (untrackedFiles.length > 0) {
            ...
            const untrackedSummary = untrackedFiles
                ...
                try {
                    ...
                    return `=== ${path.basename(cf.file)} === (unreadable)`;
                }
            }).join('\n\n');
...
Update the appropriate YAML module files using the Write tool.`);
        ...
        if (trackedFiles.length > 0) {
            ...
            for (const cf of trackedFiles) { // Process all tracked files — no artificial limit
                ...
                let code;
                try {
                    code = fs.readFileSync(cf.file, 'utf-8');
                }
                catch {
                    continue;
                }
...
Update the appropriate YAML module file using the Write tool if needed.`);
    ...
    const isLeafBox = (() => {
        try {
            ...
            return false;
        } // If we can't check, assume not a leaf
    })();
    if (isLeafBox) {
...
The implementation must actually work when executed. Test it mentally — trace through the main journeys and verify the code handles them.`);
    ...
    const validationResponse = await worker.call('The project output has been generated. How should we VALIDATE it?\n\n' +
        'PROJECT SPEC:\n' + spec.substring(0, 2000) + '\n\n' +
...
For EACH goal: PROVEN / PARTIAL / UNPROVEN — one line per goal.`);
    ...
    if (fs.existsSync(depsPath)) {
        const yaml = await import('js-yaml');
        const deps = yaml.load(fs.readFileSync(depsPath, 'utf-8'));
        if (deps?.dependencies) {
            for (const depName of Object.keys(deps.dependencies)) {
...
Read the source files and use Edit for targeted fixes.`);
    ...
    const handleEvent = async (depName, eventFile) => {
        if (!eventFile.endsWith('.event'))
            return;
        ...
        try {
            ...
            while (true) {
                const result = doCompile();
                const errors = result.issues.filter(i => i.severity === 'error');
                if (errors.length === 0 || errors.length >= prevErrors)
                    break;
...
// ── Helpers ──
function extractModuleNames(orgContent) {
    const names = [];
...
function findThinnestModule(result) {
    let thinnest = '';
...
function generateModuleExcerpt(moduleName, result) {
...
// ── Hierarchy ──
async function checkIfShouldSplit(orgContent, spec) {
...
async function runAsParent(engines, spec, orgContent) {
    ...
    if (allChildrenExist) {
        ...
        for (const f of specFileList) {
            ...
            for (const section of specSections) {
                ...
                const mentionsModule = moduleKeywords.some(kw => section.content.toLowerCase().includes(kw) ||
...
`;
            ...
            if (fs.existsSync(parentActors)) {
                ...
                fs.writeFileSync(path.join(childModulesDir, '_actors.yaml'), 'spec_sections: []\n\nnodes: {}\n\njourneys: {}\n');
            }
    ...
    if (fs.existsSync(crossModulePath)) {
        ...
        for (const child of childInterfaces) {
            try {
                ...
                const iface = yaml.load(content);
                if (iface)
                    externalInterfaces.set(child.name, iface);
            }

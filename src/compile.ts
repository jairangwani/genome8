 ...
 */
export function detectDestructiveEdit(
  before: CompileResult,
  after: CompileResult
): Array<{ module: string; nodesBefore: number; nodesAfter: number; lost: string[] }> {
  const results: Array<{ module: string; nodesBefore: number; nodesAfter: number; lost: string[] }> = [];

  // Build per-module node counts
  const beforeByModule = new Map<string, Set<string>>();
  for (const [full, node] of Object.entries(before.index.nodes)) {
    if (!beforeByModule.has(node.module)) beforeByModule.set(node.module, new Set());
    beforeByModule.get(node.module)!.add(full);
  }

  const afterByModule = new Map<string, Set<string>>();
  for (const [full, node] of Object.entries(after.index.nodes)) {
    if (!afterByModule.has(node.module)) afterByModule.set(node.module, new Set());
 ...
 */
export function detectFieldLoss(
  before: Map<string, ModuleFile>,
  after: Map<string, ModuleFile>
): Array<{ module: string; lostFields: string[] }> {
  const results: Array<{ module: string; lostFields: string[] }> = [];
  ...
  for (const [mod, beforeFile] of before) {
    ...
    for (const nodeName of Object.keys(beforeFile.nodes)) {
      if (!afterFile.nodes[nodeName]) {
        ...
        if (beforeNode.files && !afterNode.files) lostFields.push(`node:${nodeName}.files`);
        if (beforeNode.properties && !afterNode.properties) lostFields.push(`node:${nodeName}.properties`);
      }
    }
 ...
 */
export function compareResults(a: CompileResult, b: CompileResult): string[] {
  const diffs: string[] = [];
  ...
  for (const n of aNodes) {
    if (!b.index.nodes[n]) diffs.push(`Node ${n} in A but not B`);
  }
  for (const n of bNodes) {
    if (!a.index.nodes[n]) diffs.push(`Node ${n} in B but not A`);
  }
  ...
  for (const j of aJourneys) {
    if (!b.index.journeys[j]) {
      ...
      if (aSteps !== bSteps) diffs.push(`Journey ${j} step count differs: ${aSteps} vs ${bSteps}`);
    }
  }
...
export function loadAllModules(modulesDir: string): Map<string, ModuleFile> {
  ...
  for (const file of yamlFiles) {
    ...
    try {
      ...
      const parsed = yaml.load(raw) as ModuleFile;
      if (parsed && typeof parsed === 'object') {
        ...
        if (parsed.spec_sections && Array.isArray(parsed.spec_sections) &&
            parsed.spec_sections.some((s: unknown) => typeof s !== 'number')) {
...
function detectStaleConnections(nodes: Record<string, CompiledNode>): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
...
export function validateModuleCompleteness(
  ...
  compiledModules: Map<string, ModuleFile>
): string[] {
 ...
 */
export function detectUnlistedModules(
  modulesDir: string,
  expectedModules: string[]
): string[] {
 ...
 */
export function trackDirtyModules(
  modulesDir: string,
...
export function findDependentModules(
  ...
  for (const [, node] of Object.entries(index.nodes)) {
    if (changedSet.has(node.module)) continue; // skip the changed modules themselves
